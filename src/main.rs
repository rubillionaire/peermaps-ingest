    ...
    reader.for_each(|item| {
        match item {
            Element::DenseNode(node) => {
                ...
                // println!("debug:dense-node:id:{:?}", node.id);
                // println!("debug:dense-node:point:{:?}", point);
            ...
            Element::Node(node) => {
                ...
                // println!("debug:node:point:{:?}", point);
                let encoded = encode::node(node.id() as u64, point, &tags).unwrap();
                println!("{}", hex::encode(encoded));
            },
            Element::Relation(rel) => {
                ...
                println!("debug:relation:id:{:?}", rel.id());
                println!("debug:relation:members:{:?}", members);
